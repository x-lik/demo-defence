...
damageFlow:flux("prop", function(data)
    ...
    if (data.avoid > 0 and class.isObject(data.sourceUnit, UnitClass)) then
...
damageFlow:flux("avoid", function(data)
    ...
    if (approve) then
        if (data.avoid > math.rand(1, 100)) then
            -- 触发回避事件
            data.damage = 0
            event.syncTrigger(data.targetUnit, "unitAvoid", { sourceUnit = data.sourceUnit })
            event.syncTrigger(data.sourceUnit, "unitBeAvoid", { targetUnit = data.targetUnit })
            return
        end
    end
end)
...
damageFlow:flux("defend", function(data)
    ...
    elseif (data.defend > 0) then
        ...
        if (data.damage < 1) then
            -- 触发防御完全抵消事件
            data.damage = 0
            event.syncTrigger(data.targetUnit, "unitImmuneDefend", { sourceUnit = data.sourceUnit })
            return
        end
    end
end)
